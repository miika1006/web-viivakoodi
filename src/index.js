...
function initDetector() {
	...
	if (typeof BarcodeDetector.getSupportedFormats !== "function") {
		...
		try { detector = new BarcodeDetector({ formats: FORMATS }); return Promise.resolve(true); } catch (e) {}
		try { detector = new BarcodeDetector({ formats: ["qr_code", "ean_13", "code_128"] }); return Promise.resolve(true); } catch (e) {}
		return Promise.resolve(false);
	}
	...
	return BarcodeDetector.getSupportedFormats().then(function(supported) {
		...
		try { detector = new BarcodeDetector({ formats: want }); return true; } catch (e) { return false; }
	}).catch(function() {
		// getSupportedFormats rejected — try direct construction
		try { detector = new BarcodeDetector({ formats: FORMATS }); return true; } catch (e) { return false; }
	});
}
